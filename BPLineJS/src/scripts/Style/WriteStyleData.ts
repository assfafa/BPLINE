 ...
 * @example
 * WriteStyleData(style, target, offset);
 * @returns 无返回值。
 */
export const WriteStyleData = (style: Style, target: Float32Array<ArrayBuffer>, offset: number = 0): void => {
    ...
    target.set(
        [solid.opacity, solid.borderWidth, Align(solid.borderAlign), Number(solid.pixelAligned === "zoom")],
        offset + 20,
    );
    target.set(
        [edge.opacity, edge.width, Align(edge.borderAlign), Number(edge.pixelAligned === "zoom")],
        offset + 24,
    );
    target.set([points.opacity, Number(points.pixelAligned === "zoom"), 0, 0], offset + 28);
    target.set([wireframe.opacity, 0, 0, 0], offset + 32);
    target.set(
        [Number(solid.enabled), Number(wireframe.enabled), Number(edge.enabled), Number(points.enabled)],
        offset + 36,
    );
};
/**
 ...
 * @returns 0、0.5 或 1
 */
const Align = (value: "inset" | "normal" | "outset"): number => {
    // 内边框不向轮廓外扩，传给着色器的对齐系数为零。
    if (value === "inset") {
        return 0;
    } else {
        // 中边框向两侧各扩一半，其余外边框使用完整宽度。
        if (value === "normal") {
            return 0.5;
        } else {
            return 1;

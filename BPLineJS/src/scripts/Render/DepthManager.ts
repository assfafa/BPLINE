...
class DepthManager {
     ...
     * @example
     * const depthManager = new DepthManager(pipeline);
     * @returns 创建的 DepthManager 对象。
     */
                ...
                if (count > 0 && material !== undefined) {
                    ...
                    if (cached === undefined) {
                        ...
                        const buffer = device.createBuffer({
                            label: "Mesh " + String(mesh.id) + " Depth Parameters",
                            size: 16,
     ...
     * @example
     * depthManager.ensureTexture(width, height);
